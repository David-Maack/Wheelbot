...
| **Universe refresh** | Opus | Weekly, Sat 07:00 MDT (cron) | Proposes watchlist add/keep/drop per strategy. **Human must approve** (MCP `approve_watchlist`); code-enforced guardrails: open/pinned symbols undroppable, ≤2 adds + ≤2 drops per strategy, min 3 symbols, banned/tier-3 never re-addable | Last-good watchlist stands |
