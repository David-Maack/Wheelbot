"""Bull put credit spread selector.

Picks a (short_put, long_put) pair on one underlying:

  1. Run the same chain filter the CSP selector uses to find candidate
     short legs (delta band, OI/volume, spread, DTE band).
  2. Rank short candidates by annualized premium yield (highest first).
  3. For each, look for a long put at strike = short.strike - wing_width
     in the same expiry. If exact strike isn't listed, take the nearest
     strike at or below the target.
...
class SpreadCandidate:
    ...
    long: OptionContract
    net_credit_per_spread: float    # dollars per share, positive = credit
    max_loss_per_spread: float      # dollars total per package
    width_dollars: float            # short.strike - long.strike, positive
    short_yield: float              # annualized yield of the short leg (ranking only)
...
def _find_long_leg(
    ...
    """Pick the long leg for a bull put spread: strike = short - wing_width.

    Falls back to the nearest available strike at or below the target so
    ...
    if not same_expiry:
        return None
    # Prefer exact match, otherwise the nearest strike at or below target.
    same_expiry.sort(key=lambda c: c.strike, reverse=True)  # highest strike first
    at_or_below = [c for c in same_expiry if c.strike <= target]
    if at_or_below:
        # Closest from below = highest strike that is ≤ target.
        return at_or_below[0]
    # All available strikes are above target — take the lowest available
    # (smallest possible width) so we still have *some* long protection.
    return same_expiry[-1]


async def select_bull_put_spread(
    broker: Broker,
    symbol: str,
    params: dict[str, Any],
    *,
    ...
    """Return the best bull-put spread, or None if nothing passes."""
    today = today or date.today()

    # IVR soft filter (Sprint 12 sub-sprint 5). Skip when no provider, no
    ...
    filters = ChainFilters(
        ...
        delta_min=float(params.get("short_delta_min", params.get("csp_delta_min", 0.20))),
        delta_max=float(params.get("short_delta_max", params.get("csp_delta_max", 0.30))),
    ...
    short_candidates = await fetch_filtered_chain(
        broker, symbol, "put", filters, today=today
    )
    if record_chain is not None:
        await record_chain(symbol, "put", short_candidates)
    if not short_candidates:
        log_checkpoint("spread_no_short_candidates", status="ok", symbol=symbol)
        return None

    # Pull the wider chain (no delta filter) once so we can find long legs that
    # would otherwise be filtered out by the short-leg delta band.
    ...
    full_chain = await fetch_filtered_chain(
        broker, symbol, "put", wide_filters, today=today
    )
    ...
    if not scored:
        log_checkpoint("spread_no_short_scored", status="ok", symbol=symbol)
        return None
    ...
    log_checkpoint("spread_no_viable_pair", status="ok", symbol=symbol)
    return None
